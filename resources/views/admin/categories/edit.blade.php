            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="table-scroll">
                        <h1 class="Category_List">{{ trans('translation.edit_category') }}</h1>
                        <form method="POST" enctype="multipart/form-data" action="{{ route('categories.update', [$category->id]) }}">
                            ...
                            <div class="form-div">
                                <div class="mar-input">
                                    ...
                                    <input class="api-input" type="text" name="title" id="title" value="{{ $category->title }}">
                                    @if($errors->has('title'))
                                ...
                                <div class="mar-input">
                                    ...
                                    @foreach(App\Models\Category::STATUS_SELECT as $key => $label)
                                        <option value="{{ $key }}" {{ $category->status === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                    </select>
