<div class="m-3">
    @can('prompt_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route_relative('admin.prompts.create') }}">
                    ...
                    <tbody>
                        @foreach($prompts as $key => $prompt)
                            <tr data-entry-id="{{ $prompt->id }}">
                                ...
                                <td>
                                    @can('prompt_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route_relative('admin.prompts.show', $prompt->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('prompt_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route_relative('admin.prompts.edit', $prompt->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('prompt_delete')
                                        <form action="{{ route_relative('admin.prompts.destroy', $prompt->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('prompt_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route_relative('admin.prompts.massDestroy') }}",
