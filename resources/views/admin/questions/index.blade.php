        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="category-top">
                    <a href="{{ route('questions.create') }}" class="btn btn-primary Add-Category">{{ trans('translation.add_question') }}</a>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="table-scroll">
                    <h1 class="Category_List">{{ trans('translation.question_list') }}</h1>
                    <div class="card-body">
                            ...
                            <table class="table table-bordered table-striped table-hover datatable datatable-Question">
                                <thead>
                                    <tr>
                                        <th width="10"></th>
                                        <th>ID</th>
                                        <th style="width: 30%">{{ trans('translation.question') }}</th>
                                        <th style="width: 20%">{{ trans('translation.type') }}</th>
                                        <th style="width: 20%">{{ trans('translation.required') }}</th>
                                        <th style="width: 15%">{{ trans('translation.minimum_answer_length') }}</th>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let deleteButton = {
            ...
            url: "{{ route('questions.massDestroy') }}",
        ...
        let table = $('.datatable-Question').DataTable({
            dom: 'Bfrtip',
            ...
            columns: [
                ...
                { data: "type" },
                { data: "is_required" },
                { data: "minimum_answer_length" },
                {
                    data: null,
                    render: function (data, type, row) {
                        let actions = '';
                        let confirmText = 'return confirm("{{ trans('translation.are_you_sure') }}")';
                        actions += ' <a class="btn btn-xs btn-info" href="/admin/questions/' + row.id + '/edit">{{ trans('translation.edit') }}</a>';
                        actions += ' <form action="/admin/questions/' + row.id + '" method="POST" ' +
