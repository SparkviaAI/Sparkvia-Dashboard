            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="table-scroll">
                        <h1 class="Category_List">{{ trans('translation.edit_question') }}</h1>
                        <form method="POST" enctype="multipart/form-data" action="{{ route('questions.update', [$question->id]) }}">
                            ...
                            <div class="form-div">
                                <div class="mar-input">
                                    ...
                                    <input class="api-input" type="text" name="question" id="question" required value="{{ $question->question }}">
                                ...
                                <div class="mar-input">
                                    ...
                                    <select class="api-input" name="type" id="question_type" required>
                                        ...
                                        @foreach(App\Models\Question::TYPE_SELECT as $key => $label)
                                            <option value="{{ $key }}" {{ $question->type === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @if($errors->has('type'))
                                ...
                                <div class="mar-input">
                                    ...
                                    <option value disabled>Please Select</option>
                                        @foreach(App\Models\Question::IS_REQUIRED_SELECT as $key => $label)
                                            <option value="{{ $key }}" {{ $question->is_required === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>                    
                                ...
                                <div class="mar-input" id="minimum_answer_length_group">
                                    ...
                                    value="{{ $question->minimum_answer_length }}" step="1" required>                           
